...
try {
    $count_stmt = $db->prepare("SELECT COUNT(*) as total FROM products WHERE category = 'fungicide' AND is_active = 1");
    $count_stmt->execute();
    $total_products = $count_stmt->fetch()['total'];
    $total_pages = ceil($total_products / $per_page);

    $stmt = $db->prepare("SELECT * FROM products WHERE category = 'fungicide' AND is_active = 1 ORDER BY id DESC, created_at DESC LIMIT ? OFFSET ?");
...
<head>
    ...
    <style>
        ...
        .bg-blur-image {
            ...
            background-image: url('src/fungos_nas_plantas.jpg');
